...
from expl import dox_inspired_pipeline_presplit
import time
...
if __name__ == "__main__":
    ...
    results = evaluate_all_datasets_simplified_dox(
        all_datasets,
        base_dir='datasets',
        output_dir='dox_inspired_results',
    ...
    if results is not None:
        ...
        print("="*80)
        print("\nResults saved in: dox_inspired_results/")
        print("  • Individual CSV files per dataset")
        print("  • combined_dox_inspired_results.csv (all datasets)")
        print("\nNext step: Visualize results with visualize_simplified_dox.py")
        ...
        # Average DoX by condition
        results['condition'] = results['dataset_name'].apply(
            lambda x: 'baseline' if x == 'baseline' else
                     'bias' if 'hist_bias' in x else
                     'imbalance' if 'undersample' in x else
                     'noise' if 'label_noise' in x else 'other'
        )
        
        avg_by_condition = results.groupby('condition')['mean_dox'].mean()
        print("\nAverage DoX by Condition:")
        for cond in ['baseline', 'bias', 'imbalance', 'noise']:
            if cond in avg_by_condition:
                score = avg_by_condition[cond]
                print(f"  {cond.capitalize():12s}: {score:.4f}")
        
        # Largest gaps
        gap_by_dataset = results.groupby('dataset_name')['dox_gap'].first().sort_values(ascending=False)
        print("\nTop 5 Datasets with Largest DoX Gaps:")
        for dataset, gap in gap_by_dataset.head(5).items():
            print(f"  {dataset:<40s}: {gap:.4f}")
        
    else:
        print("\n✗ Evaluation failed or no results")